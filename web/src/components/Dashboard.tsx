...
export default function Dashboard({ scope }: { scope: Scope }) {
  ...
  const [sortByCap, setSortByCap] = useState(false);
  const hourly = scope.kind === "day";

  const scopeKey = scope.kind === "day" ? `day:${scope.date}` : "range";

  useEffect(() => {
    const next: Scope =
      scopeKey === "range" ? { kind: "range" } : { kind: "day", date: scopeKey.slice(4) };
    loadAll(next)
      .then(setData)
  ...
  const shown = useMemo(() => {
    if (!data) return [];
    const rows = data.feed.filter((t) => matches(t, filter));
    if (!sortByCap) return rows;
    return [...rows].sort((a, b) => b.scores.cap_score - a.scores.cap_score);
  }, [data, filter, sortByCap]);
  ...
  if (!data) {
    return (
      <main className="p-16 text-sm text-slate-500">
        {hourly ? "Loading the day…" : "Loading the labeled window…"}
      </main>
    );
  }
  ...
  return (
    <main className="min-h-screen">
      <header className="border-b border-slate-800 px-5 py-3">
        ...
        <SiteNav current={hourly ? "day" : "range"} />
      </header>

      {hourly ? (
        <DayStrip hourly={data.hourly} meta={data.meta} />
      ) : (
        <RangeStrip meta={data.meta} />
      )}
          ...
          <div className="border-b border-slate-800 px-4 py-2.5">
            <div className="flex items-start gap-3">
              ...
              <div className="ml-auto flex shrink-0 items-center gap-2">
                <button
                  type="button"
                  aria-pressed={sortByCap}
                  onClick={() => {
                    setSortByCap((on) => !on);
                    setLimit(PAGE);
                  }}
                  className={`rounded border px-2 py-1 text-[11px] hover:bg-slate-800 ${
                    sortByCap
            ...
            <p className="mt-1 text-[11px] text-slate-600">
              ...
              {sortByCap ? " Highest cap score first." : ""}
            </p>
          </div>
      ...
      <footer className="border-t border-slate-800 px-5 py-4 text-[11px] leading-relaxed text-slate-600">
        {hourly ? (
          ...
          <>
            Labeled GLP-1 posts from {data.meta.start_utc} to {data.meta.end_utc} UTC:{" "}
            {fmt.format(data.meta.slice_rows)} posts collapsed into{" "}
            {fmt.format(data.feed.length)} distinct claims. Same Cap scores as the day demo;
            hour-by-hour charts live on that page only. Scores and model labels are not
            medical or legal judgements.
          </>
        )}
      </footer>
    </main>
